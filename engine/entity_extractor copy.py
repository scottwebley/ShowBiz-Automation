...
ORGANIZATIONS = {
    ...
    # Companies frequently appearing in entertainment news
    "Comcast",
    "Google",
    "Alphabet",
    "Meta",
...
def extract_entities(
    ...
    for org in sorted(
        ...
        if org.lower() in headline_lower:
            ...
            # IMPORTANT:
            # Remember every word in the matched organization
            # so they never become generic keywords later.
            for token in re.findall(r"[A-Za-z0-9+&']+", org):
                matched_org_words.add(token.lower())
    ...
    for title in sorted(
        ...
        if title.lower() in headline_lower:
            entities["tv_shows"].append(title)
